...
public class BookServiceImpl implements BookService {
	...
	public List<Book> getBookDetails() {
		ResponseEntity<Book[]> response = restTemplate.getForEntity("http://localhost:8065/getBookDetails",
				Book[].class);
	...
	public List<Book> getBookDetailsById(int bookCode) {
		ResponseEntity<Book[]> response = restTemplate
				.getForEntity("http://localhost:8065/getBookDetailsById/" + bookCode, Book[].class);
		return Arrays.asList(response.getBody());
	}
